...
{
   ld G = 6.6743e-11;  
   ld m1,m2; //ld stands for long double
   ld pos_m1[3],pos_m2[3]; //stores the coordinates of the point mass
   cin >> m1 >> m2; //getting the masses
   for (int i=0;i<3;i++){cin >> pos_m1[i];} // getting the coordinates
   for (int i=0;i<3;i++){cin >> pos_m2[i];}
   ld f_1_2[3]; //force vector
   ld d_12; //distance between 1 and 2
